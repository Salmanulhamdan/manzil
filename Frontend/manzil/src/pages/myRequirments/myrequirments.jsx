...
function MyrequirmentListing() {
  ...
  const [intrestModalStates, setIntrestModalStates] = useState({});
  // Function to open the modal for a specific question
  const openIntrestsModal = (requirmentId) => {
    setIntrestModalStates((prevStates) => ({
      ...prevStates,
      [requirmentId]: true,
    }));
  };
  ...
  const closeIntrestsModal = (requirmentId) => {
    setIntrestModalStates((prevStates) => ({
      ...prevStates,
      [requirmentId]: false,
    }));
        ...
        {requirmentlist.map((requirment, index) => (
          ...
          <div className='save-btn ml-4' onClick={() =>{ handlesave(requirment.id);setTrigger(true);}}>
            <FontAwesomeIcon icon={faShare} color= {requirment.is_saved ? 'blue':'black'}/>
              ...
              <button className='hover:bg-gray-200 text-black font-bold py-2 px-4 rounded' onClick={() => openIntrestsModal(requirment.id)}>
                {requirment.is_intrest_confirmed ? `${requirment.is_intrest_confirmed.username} Intrest confirmed` : 'View Intrests'}

              </button>
              


              <IntrestsModal requirment={requirment.description} requirment_id={requirment.id} isOpen={intrestModalStates[requirment.id]} onClose={() => closeIntrestsModal(requirment.id)} />
            </div>
          </div>
        ))}
      </div>
